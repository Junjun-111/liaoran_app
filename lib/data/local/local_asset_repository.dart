import 'dart:async';
import 'dart:convert';

import 'package:shared_preferences/shared_preferences.dart';

import '../../domain/models/asset.dart';
import '../../domain/repositories/asset_repository.dart';

/// 资产仓储的本地持久化实现。
///
/// 数据保存到手机本地 SharedPreferences，应用重启后自动恢复。
class LocalAssetRepository implements AssetRepository {
  LocalAssetRepository._();

  static final LocalAssetRepository instance = LocalAssetRepository._();

  static const _storageKey = 'liaoran_assets_v1';

  final List<Asset> _items = [];
  bool _loaded = false;

  @override
  List<Asset> get items => List.unmodifiable(_items);

  @override
  bool get isEmpty => _items.isEmpty;

  /// 启动时从手机本地读取资产。
  @override
  Future<void> load() async {
    if (_loaded) return;
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_storageKey);
      if (raw != null && raw.isNotEmpty) {
        final decoded = jsonDecode(raw) as List<dynamic>;
        _items
          ..clear()
          ..addAll(
            decoded.map(
              (item) => Asset.fromJson(item as Map<String, dynamic>),
            ),
          );
      }
    } catch (_) {
      // 读取失败时保留空内存数据，不影响 App 启动。
    }
    _loaded = true;
  }

  @override
  void add(Asset asset) {
    _items.insert(0, asset);
    unawaited(_persist());
  }

  @override
  void replaceAll(List<Asset> assets) {
    _items
      ..clear()
      ..addAll(assets);
    unawaited(_persist());
  }

  @override
  void update(Asset asset) {
    final index = _items.indexWhere((a) => a.id == asset.id);
    if (index >= 0) {
      _items[index] = asset;
      unawaited(_persist());
    }
  }

  @override
  void remove(Asset asset) {
    _items.remove(asset);
    unawaited(_persist());
  }

  @override
  void clear() {
    _items.clear();
    unawaited(_persist());
  }

  Future<void> _persist() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = jsonEncode([
        for (final asset in _items) asset.toJson(),
      ]);
      await prefs.setString(_storageKey, raw);
    } catch (_) {
      // 写入失败时静默处理，避免影响页面操作。
    }
  }
}
